...
var VALIDELEMENTS = /^(address|article|aside|audio|blockquote|canvas|code|dd|div|dl|fieldset|figcaption|figure|footer|form|h1|h2|h3|h4|h5|h6|header|hgroup|hr|noscript|ol|output|p|pre|section|table|tfoot|ul|video|li)$/i;
						...
						if(!_isReadonly && !BLOCKED_KEYS.test(event.keyCode)){
							...
							if(_defaultVal !== '' && event.keyCode === 13){
								if(!event.shiftKey){
									...
									var selection = taSelection.getSelectionElement();
									selection.innerHTML.trim()
									while(!selection.tagName.match(VALIDELEMENTS) && selection !== element[0]){
										selection = selection.parentNode;
									}
									if(selection.tagName.toLowerCase() !== attrs.taDefaultWrap && selection.tagName.toLowerCase() !== 'li' && (selection.innerHTML.trim() === '' || selection.innerHTML.trim() === '<br>' || selection.parentNode.firstChild.tagName === 'CODE')){			
										var _new = angular.element(_defaultVal);
										if(selection.parentNode.tagName === 'PRE'){
											angular.element(selection.parentNode).replaceWith(_new);
										} else {
		...
		return {
			...
			link: function(scope, element, attrs, ngModel){
				...
				angular.extend(scope, angular.copy(taOptions), {
                    ...
                    wrapSelection: function(command, opt, isSelectableElementTool, name) {
                        // we restore the saved selection that was saved when focus was lost
                        /* NOT FUNCTIONAL YET */
						/* textAngularManager.restoreFocusSelection(scope._name, scope); */
						if(isSelectableElementTool === 'quote'){
							$(window.getSelection().focusNode).addClass('quote-cms')
						}
                        if(command.toLowerCase() === "undo"){
                            scope['$undoTaBindtaTextElement' + _serial]();
                        }else if(command.toLowerCase() === "redo"){
                            scope['$redoTaBindtaTextElement' + _serial]();
                        } else if (isSelectableElementTool === 'ulCustom') {
							_taExecCommand(command, false, opt, scope.defaultTagAttributes);

							switch(window.getSelection().focusNode.localName){
								case 'li':
									var listId = window.getSelection().focusNode;
									$(listId).addClass(opt.css);
									break;
								case 'ul':
									var eleDel = window.getSelection().focusNode.firstChild.firstChild;
									var listId = window.getSelection().focusNode.firstChild;     
									if(window.getSelection().focusNode.firstChild.firstChild.localName == 'p') {
										$(eleDel).remove();
									}
									$(listId).addClass(opt.css);
									break; 
								default:
									$(window.getSelection().focusNode.parentElement.closest('li')).addClass(opt.css);
							}
                        } else if(command === 'addBorder') {
                                if(window.getSelection().focusNode.parentElement.localName == 'li') {
                                    $(window.getSelection().focusNode.parentElement).addClass(opt.class)
                                } else {
                                    $(window.getSelection().focusNode.parentElement).removeClass().addClass(opt.class)
                                }
						} else if(command === 'inlineUl'){
								$(window.getSelection().focusNode.parentElement.closest('ul')).toggleClass('inline');
						} else if(command === 'fontSize'){
							var selection = window.getSelection().getRangeAt(0);
							var selectedText = selection.extractContents();
							var span = document.createElement('span');
							span.style = 'font-size:' + opt;
							span.appendChild(selectedText)
							selection.insertNode(span);
						} else if(command === 'uiAce'){
							if(window.getSelection().focusNode.tagName === 'P'){
								window.getSelection().focusNode.remove();
							}
							var createPre = document.createElement('pre');
							$(createPre).addClass('code').attr('data-title', opt);
							var createCode = document.createElement('code');

							if(window.getSelection().focusNode.nodeName === '#text' && window.getSelection().focusNode.parentNode.nodeName !== 'CODE') {
								createPre.appendChild(createCode).innerHTML = window.getSelection().focusNode.textContent;
								window.getSelection().focusNode.remove()
								element[0].children[1].children[2].appendChild(createPre)
								taSelection.getSelectionElement().remove();				
							} else if(window.getSelection().focusNode.parentNode.nodeName === 'CODE'){
									return
	
							} else {
								createPre.appendChild(createCode).innerHTML = '&nbsp'
								element[0].children[1].children[2].appendChild(createPre)
							}
						} else {
